...
function TransferStockModal({ stockItem, onClose, onTransferComplete }) {
    ...
    const handleTransfer = async (e) => {
        e.preventDefault();
        try {
            await transferProductStock(stockItem.productId, stockItem.gymId, parseInt(toGymId), parseInt(quantity));
            onTransferComplete();
            onClose();
        } catch (error) {
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
            <div className="bg-white p-6 rounded-lg w-full max-w-md">
                ...
                <form onSubmit={handleTransfer}>
                    ...
                    <p className="mb-4">Academia de Origem: {stockItem.gymName}</p>
                    <p className="mb-4">Estoque Disponível: {stockItem.quantity}</p>
                    <div className="mb-4">
                        <label className="block text-sm font-medium text-gray-700">Academia de Destino</label>
                        <select 
                            value={toGymId} 
