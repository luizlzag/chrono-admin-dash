...
function AddSaleModal({ onClose, onSaleAdded }) {
    ...
    useEffect(() => {
        const fetchStock = async () => {
            if (formData.productId && formData.gymId) {
                try {
                    const stockData = await getProductStockInGym(
                        formData.productId,
                        formData.gymId
                    )
    ...
    const handleQuantityChange = (e) => {
        ...
        if (stockAvailable !== null && quantity > stockAvailable) {
            alert(
                `A quantidade solicitada excede o estoque disponível (${stockAvailable}).`
            )
            quantity = stockAvailable
        }

        const selectedProduct = products.find(
            (product) => product.id === parseInt(formData.productId)
        )
        setFormData((prevData) => ({
            ...prevData,
            quantity,
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="mb-4">
                        ...
                        <input
                            ...
                            disabled={
                                stockAvailable === 0 || stockAvailable === null
                            }
                        />
                    </div>
                    {/* Campo para total */}
