...
function ListGym() {
    ...
    return ( 
        <div>
            ...
            <div className="container mx-auto p-4 mt-20">
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full bg-white border border-gray-200">
                        ...
                        <tbody>
                            {gyms.map((gym) => (
                                <tr key={gym.id} className="hover:bg-gray-100 transition-colors">
                                    <td className="py-2 px-4 border-b">{gym.name}</td>
                                    <td className="py-2 px-4 border-b">
                                        {gym.street ? (
                                            <p>
                                                {gym.street}, {gym.number} {gym.complement && ` - ${gym.complement}`}<br />
                                                {gym.neighborhood}, {gym.city}, {gym.state} - {gym.zipCode}
                                            </p>
                                        ) : (
            ...
            {isGymModalOpen && <AddGymModal onClose={toggleGymModal} onGymAdded={fetchGyms} />}
            {isUserModalOpen && <AddUserModal onClose={toggleUserModal} onUserAdded={fetchGyms} gyms={gyms} />}
        </div>
    );
}
